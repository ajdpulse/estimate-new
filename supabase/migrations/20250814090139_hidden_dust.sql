/*
  ...
  1. New Tables
    - `measurement_book`
      ...
      - `item_id` (text, references subwork_items)
...
DO $$
BEGIN
  -- Add foreign key to works table if it exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'estimate' AND table_name = 'works') THEN
    ALTER TABLE measurement_book ADD CONSTRAINT fk_measurement_book_work_id 
    FOREIGN KEY (work_id) REFERENCES estimate.works(works_id) ON DELETE CASCADE;
  END IF;

  -- Add foreign key to subworks table if it exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'estimate' AND table_name = 'subworks') THEN
    ALTER TABLE measurement_book ADD CONSTRAINT fk_measurement_book_subwork_id 
    FOREIGN KEY (subwork_id) REFERENCES estimate.subworks(subworks_id) ON DELETE CASCADE;
  END IF;

  -- Add foreign key to subwork_items table if it exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'estimate' AND table_name = 'subwork_items') THEN
    ALTER TABLE measurement_book ADD CONSTRAINT fk_measurement_book_item_id 
...
-- Admin policies (for users with admin roles)
...
COMMENT ON TABLE measurement_book IS 'Stores actual measurements for approved works in the measurement book process';
COMMENT ON COLUMN measurement_book.sr_no IS 'Serial number primary key';
COMMENT ON COLUMN measurement_book.work_id IS 'Reference to the main work';
COMMENT ON COLUMN measurement_book.subwork_id IS 'Reference to the subwork';
COMMENT ON COLUMN measurement_book.item_id IS 'Reference to the subwork item';
COMMENT ON COLUMN measurement_book.measurement_sr_no IS 'Sequential number within the item measurements';
COMMENT ON COLUMN measurement_book.estimated_quantity IS 'Original estimated quantity from the estimate';
COMMENT ON COLUMN measurement_book.actual_quantity IS 'Actual measured quantity';
COMMENT ON COLUMN measurement_book.variance IS 'Difference between actual and estimated (actual - estimated)';
COMMENT ON COLUMN measurement_book.variance_reason IS 'Explanation for significant variances';
COMMENT ON COLUMN measurement_book.measured_by IS 'Email of the user who recorded the measurement';
COMMENT ON COLUMN measurement_book.measured_at IS 'Timestamp when the measurement was taken';