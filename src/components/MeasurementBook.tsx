...
const MeasurementBook: React.FC = () => {
  ...
  const filteredMeasurements = measurements.filter(measurement => {
    const matchesSearch = measurement.description_of_items?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         measurement.subwork_item_id?.toString().toLowerCase().includes(searchTerm.toLowerCase());
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredMeasurements.map((measurement) => {
                    ...
                    const varianceStatus = getVarianceStatus(measurement.variance || 0);
                      ...
                      <tr key={measurement.sr_no} className="hover:bg-gradient-to-r hover:from-violet-50 hover:to-purple-50 transition-all duration-200">
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            <input
                              type="number"
                              value={measurement.no_of_units || 1}
                              onChange={(e) => {
                                const updated = measurements.map(m => 
                                  m.sr_no === measurement.sr_no 
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            (measurement.length || 0).toFixed(3)
                          )}
                        </td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            <input
                              type="number"
                              value={measurement.width_breadth || 0}
                              onChange={(e) => {
                                const updated = measurements.map(m => 
                                  m.sr_no === measurement.sr_no 
                            ...
                            (measurement.width_breadth || 0).toFixed(3)
                          )}
                        </td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            <input
                              type="number"
                              value={measurement.height_depth || 0}
                              onChange={(e) => {
                                const updated = measurements.map(m => 
                                  m.sr_no === measurement.sr_no 
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-sm border-r border-gray-200">
                          <div className="flex items-center">
                            ...
                            <span className={`font-medium ${(measurement.variance || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {(measurement.variance || 0) >= 0 ? '+' : ''}{(measurement.variance || 0).toFixed(3)}
                            </span>
                          </div>
                        </td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 border-r border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selectedItem = newMeasurement.subwork_id ? estimateData?.subworkItems[newMeasurement.subwork_id]?.find(item => item.id === e.target.value) : undefined;
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  onClick={() => handleSaveMeasurement({
                    ...
                    subwork_item_id: newMeasurement.item_id ? parseInt(newMeasurement.item_id) : 0
                  }, true)}
